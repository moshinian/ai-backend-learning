...
| Phase 4：Java / JVM / 并发 / Spring | DOING | 25% | Spring MVC 请求链路、Tomcat 线程模型和线程池主线已完成第一轮，并能联系连接池与长事务；锁、CAS、AQS、Spring 核心和 JVM 未展开 |
| Phase 5：Redis / MQ / 分布式 | DOING | 15% | 已学习分布式任务幂等、补偿、租约和 Redis 分布式锁第一轮；Redis 基础、缓存问题、MQ 和分布式理论主线仍未完成 |
| Phase 6：项目深挖与面试表达 | DOING | 22% | RAG 已完成架构、评测和生产化设计第一轮；结算系统已有并发与一致性素材，但 2 分钟项目故事仍未闭环 |
...
6. RAG 检索评测、版本治理、权限过滤和向量模型升级设计
7. Redis 分布式锁与任务处理权边界
...
5. **冲刺计划后半程推进失衡**
   - 线程池、RAG 工程化和 Redis 分布式锁投入较深，但 MQ、Java 锁/CAS/AQS、Spring 核心和模拟面试尚无系统沉淀。
6. **RAG 设计理解超前于项目实现**
   - 已能讨论蓝绿向量索引、权限预过滤、MMR 和线上诊断，但不能包装成当前项目已落地能力。
