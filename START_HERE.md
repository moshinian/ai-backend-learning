...
14. Java 线程池与后台任务执行第一轮正在推进：线程池提交顺序、队列与拒绝、Executors 风险、任务认领、租约心跳、反压与重试
...
16. 线程池核心流程已理解，但 `keepAliveTime`、核心线程超时、线程工厂、异常处理、监控和优雅关闭尚未学习

---

## 4. 下次从哪里开始

**当前首要断点：继续 Java 线程池核心参数，学习空闲线程回收。**

**下一道训练题：`corePoolSize=8`、`maximumPoolSize=16`、`queueCapacity=100`。流量回落后，16 个线程中只有 3 个仍在执行，其余线程长期空闲。哪些线程会被回收？由哪个参数控制？**

**线程池主笔记：`backend/java/thread-pool.md`。**

**线程池会话记录：`sessions/2026-06-12-thread-pool-task-execution.md`。**
