...
5. 动态规划已过三轮，背包分类明显稳定；LCS 和编辑距离二维转移已理解，但二维下标表达、编辑距离插入来源和手写公式准确性仍需巩固
...
**当前首要断点：编辑距离一维空间优化。明确一维数组中更新前的 `dp[j]`、更新后的 `dp[j-1]`、额外变量 `prev` 分别对应二维表的上方、左方、左上。**
...
如果用户想继续算法，直接从编辑距离一维空间优化继续。先让用户判断每次更新前为什么要保存 `temp=dp[j]`，以及本轮结束后 `prev` 为什么更新为 `temp`。
