...
16. 线程池生命周期、异常处理、监控和优雅关闭已完成第一轮，但线程池与数据库连接池的容量边界仍需综合验证

---

## 4. 下次从哪里开始

**当前首要断点：继续 Java 线程池容量与数据库连接池边界的综合判断。**

**下一道训练题：RAG 入库线程池有 32 个工作线程，数据库连接池只有 20 个连接，每个任务占用 1 个连接且事务平均持续 5 秒。压测时 `activeCount=32`、连接池 `active=20`、`pending=12`、CPU=35%。这 12 个线程在做什么？为什么增加线程无法提升吞吐？是否应该直接把连接池扩大到 32？**

**线程池主笔记：`backend/java/thread-pool.md`。**

**线程池最新会话记录：`sessions/2026-06-14-thread-pool-lifecycle-monitoring.md`。**
